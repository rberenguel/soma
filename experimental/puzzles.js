...
let PUZZLES = []; // This will be populated from the fetched file

const puzzleNameEl = document.getElementById("puzzle-name");
const container = document.getElementById("container");

// Corrected parser for the Thorleif format
function parseThorleifBlock(blockText) {
    ...
    // **THE FIX IS HERE:** Remove the leading '/' before splitting to get correct dimensions.
    ...
    const grid = Array(width).fill(0).map(() => Array(height).fill(0).map(() => Array(depth).fill(0)));
...
async function loadPuzzlesAndInit() {
    try {
        const response = await fetch('puzzles.md');
        if (!response.ok) {
        ...
        puzzleNameEl.innerText = "Error loading puzzles!";
    }
}
...
function init() {
    ...
    if(PUZZLES.length > 0) {
        ...
        puzzleNameEl.innerText = "No puzzles found.";
    }
    animate();
}
...
function generatePuzzleWireframe(grid) {
    ...
    const [width, height, depth] = [grid.length, grid[0].length, grid[0][0].length];

    const barycenter = new THREE.Vector3(0, 0, 0);
    let cubeCount = 0;

    // 1. First, calculate the true barycenter of the shape from the grid data.
    for (let x = 0; x < width; x++) {
        for (let y = 0; y < height; y++) {
            for (let z = 0; z < depth; z++) {
                if (grid[x][y][z] === 1) {
    ...
    // 2. Now, create the visual cubes, positioning each one *relative* to the barycenter.
    // This centers the puzzle's mass around the group's origin (0,0,0).
    for (let x = 0; x < width; x++) {
        for (let y = 0; y < height; y++) {
            for (let z = 0; z < depth; z++) {
                if (grid[x][y][z] === 1) {
                    ...
                    // Set position relative to the barycenter
                    line.position.set(x - barycenter.x, y - barycenter.y, z - barycenter.z);
    ...
    // 3. Finally, rotate the now-centered group.
...
function addEventListeners() {
    ...
    container.addEventListener("pointerup", () => { isRotatingCamera = false; });
    container.addEventListener("pointerleave", () => { isRotatingCamera = false; });
}
